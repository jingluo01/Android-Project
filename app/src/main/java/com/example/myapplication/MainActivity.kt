...
class MainActivity : AppCompatActivity() {
    ...
    var accounService:AccounService = AccounService(this)
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        accounService.setAdapter(listView)
...
//        listView.adapter = accountAdapter
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        ...
        var index: Int = menuInfo.id.toInt()
//        var account: Account = accountList[id]
        when (item.itemId) {
            1 -> accounService.deleteAccount(index)
            2 -> accounService.updataAccount(index)
            3 -> accounService.selectAccount()
        }
...
//    }
    fun add(view: View) {
        ...
        accounService.addAccount(account)
        clear()
    }
    fun clear() {
        et_name.text = null
